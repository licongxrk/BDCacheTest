...
@class BDDiskCache;
@protocol BDCacheBackgroundTaskManager;

typedef void (^BDDiskCacheBlock)(BDDiskCache *cache);
typedef void (^BDDiskCacheObjectBlock)(BDDiskCache *cache, NSString *key, _Nullable id<NSCoding> object, NSURL *fileURL);
...
/**
 The URL of the directory used by this cache, usually `Library/Caches/com.buding.TMDiskCache.(name)`
 ...
 */
@property (copy) BDDiskCacheBlock willRemoveAllObjectsBlock;

/**
 A block to be executed just after all objects are removed from the cache as a result of <removeAllObjects:>.
 The queue waits during execution.
 */
@property (copy) BDDiskCacheBlock didRemoveAllObjectsBlock;
...
/**
 ...
 This method returns immediately and executes the passed block as soon as the cache has been trimmed.
 到缓存的大小大于count值，则进行删除缓存，从最大的缓存开始删除
 @param byteCount The cache will be trimmed equal to or smaller than this size.
 @param block A block to be executed serially after the cache has been trimmed, or nil.
 */
- (void)trimToSize:(NSUInteger)byteCount block:(BDDiskCacheBlock)block;
...
/**
 ...
 been trimmed.
 到缓存的大小大于count值，则进行删除缓存，按照日期顺序开始删除
 @param byteCount The cache will be trimmed equal to or smaller than this size.
 @param block A block to be executed serially after the cache has been trimmed, or nil.
 */
...
/**
 ...
 设置一个全局管理器，用于设置/分解TMCache所需的任何后台任务。
