...
//  BDCacheTest
//
//  Created by licong on 2019/1/10.
...
NSString * const BDMemoryCachePrefix = @"com.BDMemoryCache";
...
#else
@property (assign, strong) dispatch_queue_t queue;
#endif
...
- (instancetype)init {
    if (self = [super init]) {
        NSString *queueName = [NSString stringWithFormat:@"%@.%p",BDMemoryCachePrefix,self];
...
- (void)handleMemoryWarning {
    ...
    __weak BDMemoryCache *weakSelf = self;
    dispatch_async(_queue, ^{
        __strong BDMemoryCache *strongSelf = weakSelf;
        if (!strongSelf) {
            return;
        }
...
- (void)handlerApplicationBackground {
#if __IPHONE_OS_VERSION_MIN_REQUIRED >= __IPHONE_4_0
    if (self.removeAllObjectsOnMemoryWarning) {
        [self removeAllObjects:nil];
    }
    __weak BDMemoryCache *weakSelf = self;
    dispatch_async(_queue, ^{
        BDMemoryCache *strongSelf = weakSelf;
        if (!strongSelf) {
            return;
        }
...
- (void)removeObjectAndExecuteBlocksForKey:(NSString *)key {
    ...
    if (_willRemoveObjectBlock) {
        _willAddObjectBlock(self, key, object);
    }
    if (cost) {
...
- (void)trimToCostLimitByDate:(NSUInteger)limit {
    ...
    NSArray *keysSortedByCost = [_dates keysSortedByValueUsingSelector:@selector(compare:)];
    for (NSString *key in keysSortedByCost) { // oldest objects first
        [self removeObjectAndExecuteBlocksForKey:key];
        if (_totalCost <= limit) {
            break;
        }
...
- (void)trimToAgeLimitRecursively {
    ...
    dispatch_after(time, _queue, ^{
        ...
        __weak BDMemoryCache *weakSelf = strongSelf;
...
- (void)objectForKey:(NSString *)key block:(BDMemoryCacheObjectBlock)block {
...
- (void)setObject:(id)object forKey:(NSString *)key block:(BDMemoryCacheObjectBlock)block {
    [self setObject:object foKey:key withCost:0 block:nil];
}

- (void)setObject:(id)object foKey:(NSString *)key withCost:(NSUInteger)cost block:(BDMemoryCacheObjectBlock)block {
    NSDate *now = [[NSDate alloc] init];
    if (!key || !object) {
        return;
    }
...
- (void)removeObjectForKey:(NSString *)key block:(BDMemoryCacheObjectBlock)block {
    if (!key) {
        return;
    }
    __weak BDMemoryCache *weakSelf = self;
...
- (void)trimToDate:(NSDate *)date block:(BDMemoryCacheBlock)block {
    if (!date) {
        return;
    }
    ...
    dispatch_barrier_async(_queue, ^{
        ...
        [strongSelf trimMemoryToDate:date];
        if (block) {
            __weak BDMemoryCache *weakSelf = strongSelf;
            dispatch_async(strongSelf->_queue, ^{
                BDMemoryCache *strongSelf = weakSelf;
...
- (void)trimToCost:(NSUInteger)cost block:(BDMemoryCacheBlock)block {
    __weak BDMemoryCache *weakSelf = self;
    dispatch_barrier_async(_queue, ^{
        BDMemoryCache *strongSelf = weakSelf;
        if (!strongSelf) {
...
- (void)trimToCostByDate:(NSUInteger)cost block:(BDMemoryCacheBlock)block {
    __weak BDMemoryCache *weakSelf = self;
    dispatch_barrier_async(_queue, ^{
        BDMemoryCache *strongSelf = weakSelf;
        if (!strongSelf) {
...
- (void)removeAllObjects:(BDMemoryCacheBlock)block {
    __weak BDMemoryCache *weakSelf = self;
    dispatch_barrier_async(_queue, ^{
        BDMemoryCache *strongSelf = weakSelf;
        if (!strongSelf) {
...
- (void)enumrateObjectsWithBlock:(BDMemoryCacheObjectBlock)block completionBlock:(BDMemoryCacheBlock)completionBlock {
    if (!block) {
        return;
    }
    __weak BDMemoryCache *weakSelf = self;
...
- (id)objectForKey:(NSString *)key {
    ...
    [self objectForKey:key block:^(BDMemoryCache *cache, NSString *key, id object) {
        objectForKey = object;
...
- (void)setObject:(id)object forKey:(NSString *)key withCost:(NSUInteger)cost {
    ...
    dispatch_semaphore_t semaphore = dispatch_semaphore_create(0);
    [self setObject:object foKey:key withCost:cost block:^(BDMemoryCache *cache, NSString *key, id object) {
        dispatch_semaphore_signal(semaphore);
    }];
    dispatch_semaphore_wait(semaphore, DISPATCH_TIME_FOREVER);
#if !OS_OBJECT_USE_OBJC
...
- (void)removeObjectForKey:(NSString *)key {
    ...
    dispatch_semaphore_t semaphore = dispatch_semaphore_create(0);
    [self removeObjectForKey:key block:^(BDMemoryCache *cache, NSString *key, id object) {
        dispatch_semaphore_signal(semaphore);
    }];
    dispatch_semaphore_wait(semaphore, DISPATCH_TIME_FOREVER);
#if !OS_OBJECT_USE_OBJC
...
- (void)trimToDate:(NSDate *)date {
    ...
    dispatch_semaphore_t semaphore = dispatch_semaphore_create(0);
    [self trimToDate:date block:^(BDMemoryCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
    dispatch_semaphore_wait(semaphore, DISPATCH_TIME_FOREVER);
#if !OS_OBJECT_USE_OBJC
...
- (void)trimToCost:(NSUInteger)cost {
    dispatch_semaphore_t semaphore = dispatch_semaphore_create(0);
    [self trimToCost:cost block:^(BDMemoryCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
    dispatch_semaphore_wait(semaphore, DISPATCH_TIME_FOREVER);
#if !OS_OBJECT_USE_OBJC
...
- (void)trimToCostByDate:(NSUInteger)cost {
    dispatch_semaphore_t semaphore = dispatch_semaphore_create(0);
    [self trimToCostByDate:cost block:^(BDMemoryCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
    dispatch_semaphore_wait(semaphore, DISPATCH_TIME_FOREVER);
#if !OS_OBJECT_USE_OBJC
...
- (void)removeAllObjects {
    dispatch_semaphore_t semaphore = dispatch_semaphore_create(0);
    [self removeAllObjects:^(BDMemoryCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
    dispatch_semaphore_wait(semaphore, DISPATCH_TIME_FOREVER);
#if !OS_OBJECT_USE_OBJC
...
- (void)enumrateObjectsWithBlock:(BDMemoryCacheObjectBlock)block {
    if (!block) {
        return;
    }
    dispatch_semaphore_t semaphore = dispatch_semaphore_create(0);
    [self enumrateObjectsWithBlock:block completionBlock:^(BDMemoryCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
    dispatch_semaphore_wait(semaphore, DISPATCH_TIME_FOREVER);
#if !OS_OBJECT_USE_OBJC
...
- (BDMemoryCacheObjectBlock)willAddObjectBlock {
    __block BDMemoryCacheObjectBlock block = nil;
...
- (void)setWillAddObjectBlock:(BDMemoryCacheObjectBlock)willAddObjectBlock {
    __weak BDMemoryCache *weakSelf = self;
    dispatch_barrier_async(_queue, ^{
        BDMemoryCache *strongSelf = weakSelf;
        if (!strongSelf) {
...
- (BDMemoryCacheObjectBlock)willRemoveObjectBlock {
    __block BDMemoryCacheObjectBlock block = nil;
    dispatch_sync(_queue, ^{
...
- (void)setWillRemoveObjectBlock:(BDMemoryCacheObjectBlock)willRemoveObjectBlock {
    __weak BDMemoryCache *weakSelf = self;
    dispatch_barrier_async(_queue, ^{
        BDMemoryCache *strongSelf = weakSelf;
        if (!strongSelf) {
...
- (BDMemoryCacheBlock)willRemoveAllObjectsBlock {
    __block BDMemoryCacheBlock block = nil;
    dispatch_sync(_queue, ^{
...
- (void)setWillRemoveAllObjectsBlock:(BDMemoryCacheBlock)willRemoveAllObjectsBlock {
    __weak BDMemoryCache *weakSelf = self;
    dispatch_barrier_async(_queue, ^{
        BDMemoryCache *strongSelf = weakSelf;
        if (!strongSelf) {
...
- (BDMemoryCacheObjectBlock)didAddObjectBlock
{
    __block BDMemoryCacheObjectBlock block = nil;
...
- (void)setDidAddObjectBlock:(BDMemoryCacheObjectBlock)block
{
    __weak BDMemoryCache *weakSelf = self;
...
- (BDMemoryCacheObjectBlock)didRemoveObjectBlock
{
    __block BDMemoryCacheObjectBlock block = nil;
...
- (void)setDidRemoveObjectBlock:(BDMemoryCacheObjectBlock)block
{
    __weak BDMemoryCache *weakSelf = self;
...
- (BDMemoryCacheBlock)didRemoveAllObjectsBlock
{
    __block BDMemoryCacheBlock block = nil;
...
- (void)setDidRemoveAllObjectsBlock:(BDMemoryCacheBlock)block
{
    __weak BDMemoryCache *weakSelf = self;
...
- (BDMemoryCacheBlock)didReceiveMemoryWarningBlock
{
    __block BDMemoryCacheBlock block = nil;
...
- (void)setDidReceiveMemoryWarningBlock:(BDMemoryCacheBlock)block
{
    __weak BDMemoryCache *weakSelf = self;
...
- (BDMemoryCacheBlock)didEnterBackgroundBlock
{
    __block BDMemoryCacheBlock block = nil;
...
- (void)setDidEnterBackgroundBlock:(BDMemoryCacheBlock)block
{
    __weak BDMemoryCache *weakSelf = self;
